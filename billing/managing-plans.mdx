...
* **Currency:** Select the Plan currency.

💡To add more available currencies, go to Settings > General > Available currencies

* **Plan Type:** Choose the type of plan from the following options:
  * Standard: Designed for Self-serve or subscriptions that customers can purchase independently. These plans are displayed in the Customer Portal and end users have the possibility to upgrade / downgrade among standard plans.
...
📌 **Applicability Rule:** Plan components (pricing, trial period, etc.) only apply from the subscription **start date**. If the plan is updated, the changes will only apply to **new customers** subscribing after the update. **Existing subscriptions will not be retroactively modified.**

* Self-service rank: Impact the display order of standard plan on the Customer Portal. 

💡 **Plans are sorted numerically**: a rank of "1" is displayed first, followed by "2", and so on. In the below example, the Standard 20 Plan had a self-service rank set at "1" and the Standard 80 Plan set at "2".
