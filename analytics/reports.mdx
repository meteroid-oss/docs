...
Use **Break down by** to split one series into many—one per dimension value. You can combine a **primary and a secondary** dimension (e.g. `US · pro`), cap the primary to a **Top N** (the rest roll up into "Other"), or split **by segment** to get one series per [saved cohort](/analytics/segments).
</Step>
<Step title="Filter">
...
**Export CSV**, **save** the exploration as a custom chart, or click **Add monitor** to open a [Steering](/analytics/steering) alert pre-filled with the chart's exact metric, filters and breakdown.
</Step>
</Steps>
