...
<Info>Something is still unclear? Don't hesitate to reach out to us at 👉 [**hey@meteroid.com**](#) for additional support.</Info>

## Add-ons 

An Add-on is an optional product or service that can be purchased in addition to a Subscription. Add-ons allow you to extend an End User's subscription with additional features, capacity, or services without requiring them to change Plans.

Depending on its configuration, an Add-on can be purchased directly by the End User through the Customer Portal or assigned manually by an Account Manager to a subscription.
...
<Info>For more details, refer to **Settings > Merchant**.</Info>
...
The Customer Portal is a self-service space where End Users can independently manage their relationship with your product. Through the Portal, End Users can:

* View, download, and pay their invoices
* Monitor the status of their subscriptions
* Update their billing information
* Add and manage payment methods

## Credit Note

A Credit Note is a commercial document issued by a seller to a buyer. It serves as a formal notification that the customer has been credited a specific amount against a sale that has already been invoiced and, critically, paid or partially paid. It effectively reduces the customer's accounts receivable balance with the seller.

## Entitlements

An Entitlement defines what a Customer is allowed to access, use, or consume as part of a Subscription. Entitlements in Meteroid can be managed in the Features section.
...
A **Purchase Order (PO) number** is a unique reference communicated by the Customer to their Account Manager. It allows the Customer's finance department to internally link the Invoice they receive back to the original contract or quote signed, facilitating efficient approval and payment processing. For many large companies, a valid PO number is mandatory to process and approve supplier invoices.
